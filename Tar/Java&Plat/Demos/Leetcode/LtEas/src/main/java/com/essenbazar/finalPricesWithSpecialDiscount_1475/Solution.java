package com.essenbazar.finalPricesWithSpecialDiscount_1475;
...
class Solution {
    ...
    private int returnDiscount(int[] prices, int i, int currPrice) {
        int temp_discount = -1;
        for (int j = i + 1; j < prices.length; j++) {
            if (prices[j] <= prices[i]) {
                temp_discount++;
                temp_discount = prices[i] - prices[j];
                break;
            }
        }
        return (temp_discount == -1 ? currPrice : temp_discount);
    }

    public int[] finalPrices(int[] prices) {
        List<Integer> list = new ArrayList<>();
        for (int i = 0; i < prices.length; i++) {
            int get_discount = returnDiscount(prices, i, prices[i]);
            list.add(get_discount);
        }
