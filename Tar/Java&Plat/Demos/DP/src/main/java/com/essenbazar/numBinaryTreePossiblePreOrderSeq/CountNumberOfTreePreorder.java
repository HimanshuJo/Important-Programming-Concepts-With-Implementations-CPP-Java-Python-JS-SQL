package com.essenbazar.numBinaryTreePossiblePreOrderSeq;

/**
 * Given a preorder sequence how many unique trees can be created Solution is
 * CATALAN NUMBER Number of tree is exactly same as number of unique BST create
 * with array of size n
 * 
 * The way it works for preorder sequence is as follows
 * 
 * Suppose our preorder sequence is 1 2 3 4 So we need to compute following
 * things count(3)* 2 (combination of 2,3 and 4 on both side of 1)
 ...
 * 
 * So our final result is count(0)*count(3) + count(1)*count(2) +
 * count(2)*count(1) + count(3)*count(0) which is a catalan number
 */
