...
where <col1> represents the column c1, <row1> represents the row r1, <col2> represents the 
...
public:
    vector<string> cellsInRange(string s) {
        vector<string>res;
        char bgnC=s[0], endC=s[3];
        int bgnN=s[1]-'0', endN=s[4]-'0';
        while(bgnC<=endC){
            for(int i=bgnN; i<=endN; ++i){
                string temp="";
                temp+=bgnC;
                temp+=to_string(i);
                res.push_back(temp);
        ...
        string endStr=s.substr(3, 2);
        int numTb=last[1]-'0';
        if(last!=endStr){
            for(int i=numTb+1; i<=endN; ++i){
                string temp="";
                temp+=endC;
                temp+=to_string(i);
                res.push_back(temp);
                if(temp==endStr) break;
                last=temp;
            }   
        }
        return res;
