...
public:
    ...
    int findNextIdx(vector<int>&possRws, int curridx){
        auto it=upper_bound(possRws.begin(), possRws.end(), curridx);
        if(it!=possRws.end()){
            return *it;
        }
        return -1;
    ...
    int numberOfBeams(vector<string>& bank) {
        ...
        int ans=0;
        if(sz>1){
            vector<int>possRws=findGoodRws(bank);
            for(auto &vals: possRws){
                int nwidx=findNextIdx(possRws, vals);
                if(nwidx==-1) continue;
                string curr=bank[vals], next=bank[nwidx];
                int len=curr.length();
                for(int x=0; x<len; ++x){
                    if(curr[x]=='1'){
                        for(int y=0; y<len; ++y){
