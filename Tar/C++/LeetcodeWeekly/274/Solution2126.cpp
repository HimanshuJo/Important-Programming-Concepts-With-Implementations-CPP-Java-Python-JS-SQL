...
public:
    bool asteroidsDestroyed(int mass, vector<int>& asteroids) {
        ...
        while(true){
            bool flag=false;
            for(int i=0; i<n; ++i){
                if(seen[i]==false&&currmas>=asteroids[i]){
                    flag=true;
                    currmas+=asteroids[i];
                    seen[i]=true;
                    cnt++;
...
public:
    bool asteroidsDestroyed(int mass, vector<int>& asteroids) {
        ...
        int n=asteroids.size();
        int cnt=0;
        if(currmas>=asteroids[n-1]) return true;
        while(true){
            auto it=lower_bound(asteroids.begin(), asteroids.end(), currmas);
            if(*it>currmas||it==asteroids.end()) it--;
            if(it>=asteroids.begin()){
                currmas+=*it;
                int idx=it-asteroids.begin();
                auto itt=asteroids.begin()+idx;
                asteroids.erase(itt);
                cnt++;
                if(cnt==n) return true;
            } else return false;
...
public:
    bool asteroidsDestroyed(int mass, vector<int>& asteroids) {
        ...
        for(auto &vals: asteroids)
            pq.push(vals);
        long long currmas=mass;
        while(!pq.empty()){
            int curr=pq.top();
            pq.pop();
            if(curr>currmas) return false;
            currmas+=curr;
        }
        return true;
    }
