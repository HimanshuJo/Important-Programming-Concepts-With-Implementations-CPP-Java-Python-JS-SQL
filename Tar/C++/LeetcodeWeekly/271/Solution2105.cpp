...
the ith plant needs, and two integers capacityA and capacityB representing the capacities of 
Alice's and Bob's watering cans respectively, return the number of times they have to refill to water all the plants.
...
public:
    ...
    {
        if ((plants[bgn] <= capacityA && capacityA == capacityB) || 
            (plants[bgn] <= capacityA && capacityA > capacityB) || 
            (plants[end] <= capacityB && capacityB > capacityA))
        {
            return true;
        }
        else if ((plants[bgn] <= capA && capacityA > capacityB) || 
                 (plants[end] <= capB && capacityB > capacityA) || 
                 (plants[bgn] <= capA && capacityA == capacityB))
        {
            return false;
        }
        return true;
