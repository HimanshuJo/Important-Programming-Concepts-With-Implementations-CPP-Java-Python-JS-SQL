...
Since nums1 and nums2 are permutation of [0, 1, 2, ..., n-1], we can alway process the arrays to make nums1 in increasing order.

for example, nums1 = [4,0,1,3,2], nums2 = [4,1,0,2,3], we can modify nums1 into [0, 1, 2, 3, 4], 
and correspondingly change elements in nums2, and nums2 becomes [0, 2, 1, 4, 3].

The key idea of this solution is, given an array nums of size n, 
for every element nums[i] in this array, count elements smaller than nums[i] in subarray nums[0: i-1], 
and count greater than in nums[i+1: n-1].

we use segment tree to maintain if we have seen an element, if so, change the value in tree to 1, 
and iteratively increase the parent node for range sum.
