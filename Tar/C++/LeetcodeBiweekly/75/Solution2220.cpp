...
from either 0 to 1 or 1 to 0.
    ...
    right to get 110, flip the second bit from the right to get 101, flip the fifth bit from the 
    right (a leading zero) to get 10111, etc.
...
public:
    ...
    int minBitFlips(int start, int goal) {
        ...
        if(start!=0)
            start1=toBinary(start);
        if(goal!=0)
            start2=toBinary(goal);
        if(start==0){
            while(start1.length()!=32){
                start1+='0';
            }
        }
        if(goal==0){
            while(start2.length()!=32){
                start2+='0';
            }
        }
        int res=0;
        for(int i=0; i<32; ++i){
            if(start1[i]!=start2[i]) res++;
        }
        return res;
    }
};