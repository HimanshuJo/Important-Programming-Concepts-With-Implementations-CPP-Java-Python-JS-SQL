...
public:
    long long sumScores(string s) {
        ...
        for(int i=sz-1; i>=0; --i){
            ...
            temp_+=s[i];
            curr.insert(0, temp_);
            if(curr[0]!=s[0]) continue;
            int idx=0, currsz=curr.size();
            int temp=0;
            while(curr[idx]==s[idx]&&idx<=currsz-1){
                idx++;
                temp++;
            }
            ans+=temp;
...
public:
    ...
    vector<int> func(string &s){
        ...
        for(int k=1; k<n; ++k){
        	/*
        	when k<=j we make use of already computed Z value for smaller index
        	*/
            if(k<=j){
                Z[k]=min(j-k+1, Z[k-i]);
            }
            /*
            if k>j nothing matches so we will calculate Z[k] using naive way
            */
