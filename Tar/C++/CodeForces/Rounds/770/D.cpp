...
and we will tell you the value of max(ai,aj,ak)−min(ai,aj,ak). In other words, we will tell you the difference between the maximum and the minimum number among ai, aj and ak.
...
Your program must terminate immediately after reading −1, and you will get a verdict Wrong answer. Otherwise you may get any verdict, 
because the program will continue reading from the closed stream. Note that if the query is correct, the answer will never be −1 because max(ai,aj,ak)−min(ai,aj,ak)≥0.

To give the final answer, print "! i j" (without the quotes). Printing the same number twice (that is, i=j) is allowed.
Note that giving this answer is not counted towards the limit of 2⋅n−2 queries. After that, your program must continue to solve the remaining test cases, or exit if all test cases have been solved.
...
The second line of each test case must contain n integers separated by spaces — a1,a2,…,an (0<=ai<=10^9). There must also be exactly one zero in this array.
