...
Each element in the array denotes a pile of candies of size candies[i]. You can divide each pile 
...
that each child gets the same number of candies. Each child can take at most one pile of candies and some 
piles of candies may go unused.
...
size 5 and 1. We now have five piles of candies of sizes 5, 5, 3, 5, and 1. We can allocate the 
3 piles of size 5 to 3 children. It can be proven that each child cannot receive more than 5 candies.
Example 2:
...
public:
    ...
    bool isValid(vector<int>&candies, int mid, long long k){
        long long count=0;
        for(auto &vals: candies){
            if(vals>mid){
                count+=vals/mid;
            } else if(vals==mid) count++;
        }
        return count>=k;
    }
