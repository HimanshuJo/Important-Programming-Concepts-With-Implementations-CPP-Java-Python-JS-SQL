...
public:
    ...
    pair<int, vector<int>> func(vector<int>&aliceArrows, int mask, int remainArrows){
        int score=0;
    ...
    vector<int> maximumBobPoints(int numArrows, vector<int>& aliceArrows) {
        ...
        for(int mask=0; mask<(1<<12); ++mask){
            pair<int, vector<int>>scoreBobArr=func(aliceArrows, mask, numArrows);
            int score=scoreBobArr.first;
            vector<int>bobArrows=scoreBobArr.second;
            if(score>bestScore){
                bestScore=score;
