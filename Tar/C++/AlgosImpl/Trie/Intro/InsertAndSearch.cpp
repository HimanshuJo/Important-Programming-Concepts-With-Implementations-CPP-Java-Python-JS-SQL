...
Inserting a key into Trie is a simple approach.
	...
	If the input key is a prefix of the existing key in Trie, we simply mark the last node of
	the key as the end of a word. The key length determines Trie depth.

-------

Searching for a key is similar to insert operation,

	however, we only compare the characters and move down.
...
all other nodes of root will be NULL.
