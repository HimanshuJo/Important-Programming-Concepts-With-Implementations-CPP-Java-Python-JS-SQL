...
public:
    ...
    int dfs(string s, int idx, int n, int memo[]){
        if(idx>n) return 0;
        if(idx==n) return 1;
        if(memo[idx]!=-1) return memo[idx];
        int sub1=stoi(s.substr(idx, 1)), sub2=stoi(s.substr(idx, 2));
        int fst=0, sec=0;
        if(sub1>=1&&sub1<=9){
            fst=dfs(s, idx+1, n, memo);
        }
        if(sub2>=10&&sub2<=26){
            sec=dfs(s, idx+2, n, memo);
        }
        int res=fst+sec;
        return memo[idx]=res;
    }
