...
So we generate 2^n possible pairs where n is number of bits in X. Then for every pair,
we check if its sum is S or not.
...
If X[i] = 1 and A[i] = 0, then (a[i] = 1 and b[i] = 0) or (a[i] = 0 and b[i] = 1),
