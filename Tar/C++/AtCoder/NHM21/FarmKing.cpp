...
struct Game {
    ...
    Action select_next_action(int day) {
        ...
        if (money < next_price) {
            ...
            int endInd=-1;
            int strtInd=-1;
            for(int i=0; i<veges_end.size(); ++i){
                vector<Vegetable>curr=veges_end[i];
                bool flag=false;
                for(const Vegetable& vege: curr){
                    if(vege.e==toChk&&has_machine[vege.r][vege.c]){
                        endInd=i;
                        flag=true;
                        break;
                    }
            ...
            for(int i=0; i<veges_start.size(); ++i){
                vector<Vegetable>curr=veges_start[i];
                bool flag=false;
                for(const Vegetable& vege: curr){
                    if(vege.s>=toChk&&!has_machine[vege.r][vege.c]){
                        strtInd=i;
                        flag=true;
                        break;
                    }
            ...
            if(endInd!=-1){
                if(strtInd!=-1){
                    ...
                    auto rw2=0;
                    auto cl2=0;
                    vector<Vegetable>curr=veges_end[endInd];
                    vector<Vegetable>curr2=veges_start[strtInd];
                    for(const Vegetable& vege: curr){
                        rw1=vege.r;
                        cl1=vege.c;
                        break;
                    }
                    for(const Vegetable& vege: curr2){
                        rw2=vege.r;
                        cl2=vege.c;
                        break;
                    }
                    return Action::move(rw1, cl1, rw2, cl2);
                }
            }
