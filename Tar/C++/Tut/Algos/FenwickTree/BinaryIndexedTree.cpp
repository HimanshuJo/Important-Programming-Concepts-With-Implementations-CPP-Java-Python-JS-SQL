...
To update a value, simply do arr[i] = x. The first operation takes O(n) time 
and the second operation takes O(1) time. 
...
Construction:
	...
	Then we call update() for all the indexes, the update() operation is discussed below.
...
Operations 
	...
	update(x, val): 
		...
		2) Do the following while the current index is smaller than or equal to n. 
			...
			…b) Go to next element of BITree[index]. 
...
arr[i] within their ranges being updated. 

We loop over such nodes in the BITree by repeatedly adding the decimal number corresponding 
to the last set bit of the current index.
...
How does Binary Indexed Tree work? 
	...
	The idea is based on the fact that all positive integers can be represented as the sum of powers of 2. 
		...
		For example, in the first diagram above (the diagram for getSum()), the sum of the first 12 elements 
		can be obtained by the sum of the last 4 elements (from 9 to 12) plus the 
		sum of 8 elements (from 1 to 8). 
