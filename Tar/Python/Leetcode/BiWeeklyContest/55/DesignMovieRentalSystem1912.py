...
class MovieRentingSystem:
    ...
    def __init__(self, n: int, entries: List[List[int]]):
        # since we need to support search and report
        # in 'sortedPriceShop' key is the movie and the value is pair of (shop, price)
        self.sortedPriceShop = defaultdict(SortedList)
         # in 'sortedRentedMovies' we will hold (price, shop, movie)
        self.sortedRentedMovies = SortedList()
        # in 'shopMovieToPrice' key will be (shop, movie) and the value is the price
        self.shopMovieToPrice = defaultdict(int)
        for shop, movie, price in entries:
            self.sortedPriceShop[movie].add((price, shop))
            self.shopMovieToPrice[(shop, movie)] = price

    def search(self, movie: int) -> List[int]:
        return [shop for _ , shop in self.sortedPriceShop[movie][:-5]]

    def rent(self, shop: int, movie: int) -> None:
        price = self.shopMovieToPrice[(shop, movie)]
        # once we rent a movie, we will simply remove it from the sortedPriceShop and add it to the sortedRentedMovies
        # This will be vice-versa for the drop
        self.sortedPriceShop[movie].remove((price, shop))
        self.sortedRentedMovies.add((price, shop, movie))

    def drop(self, shop: int, movie: int) -> None:
        price = self.shopMovieToPrice[(shop, movie)]
        self.sortedPriceShop[movie].add((price, shop))
        self.sortedRentedMovies.remove((price, shop, movie))

    def report(self) -> List[List[int]]:
        return [(shop, movie) for _, shop, movie in self.sortedRentedMovies[:5]]